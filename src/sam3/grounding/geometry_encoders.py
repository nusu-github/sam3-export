"""Geometry prompts and encoder for SAM3 grounding.

Port of ``sam3.model.geometry_encoders``.
"""
...
def activation_ckpt_wrapper(
    module: nn.Module | Callable[..., Any],
) -> Callable[..., Any]:
    """Eval-friendly no-op wrapper (matches training-disabled path)."""

    def run(*args: Any, act_ckpt_enable: bool = False, **kwargs: Any) -> Any:
        # Drop act_ckpt_enable if the module doesn't accept it
        try:
            return module(*args, **kwargs)
        except TypeError:
            kwargs.pop("act_ckpt_enable", None)
            return module(*args, **kwargs)
...
def is_right_padded(
    mask: Bool[Tensor, "batch seq"] | Integer[Tensor, "batch seq"],
) -> Bool[Tensor, ""]:
...
def concat_padded_sequences(
    seq1: Float[Tensor, "seq1 batch *rest"] | Integer[Tensor, "seq1 batch *rest"],
    mask1: Bool[Tensor, "batch seq1"] | Integer[Tensor, "batch seq1"],
    seq2: Float[Tensor, "seq2 batch *rest"] | Integer[Tensor, "seq2 batch *rest"],
    mask2: Bool[Tensor, "batch seq2"] | Integer[Tensor, "batch seq2"],
...
class Prompt:
    ...
    def __init__(
        self,
        box_embeddings: Float[Tensor, "n_boxes batch *box_rest"] | None = None,
        box_mask: Bool[Tensor, "batch n_boxes"] | None = None,
        point_embeddings: Float[Tensor, "n_points batch *point_rest"] | None = None,
        point_mask: Bool[Tensor, "batch n_points"] | None = None,
        box_labels: Integer[Tensor, "n_boxes batch"] | None = None,
        point_labels: Integer[Tensor, "n_points batch"] | None = None,
        mask_embeddings: Float[Tensor, "n_masks batch *mask_rest"] | None = None,
        mask_mask: Bool[Tensor, "batch n_masks"]
        | None = None,  # Attention mask for mask prompt
        mask_labels: Integer[Tensor, "n_masks batch"] | None = None,
    ):
    ...
    def _init_seq_len_and_device(
        self,
        box_embeddings: Tensor | None,
        point_embeddings: Tensor | None,
        mask_embeddings: Tensor | None,
    ) -> tuple[int, int, int, int | None, torch.device | None]:
    ...
    def append_boxes(
        self,
        boxes: Float[Tensor, "n batch *rest"],
        labels: Integer[Tensor, "n batch"],
    ...
    def append_points(
        self,
        points: Float[Tensor, "n batch *rest"],
        labels: Integer[Tensor, "n batch"],
        mask: Bool[Tensor, "batch n"] | None = None,
    ) -> None:
    ...
    def append_masks(
        self,
        masks: Float[Tensor, "n batch *rest"],
        labels: Integer[Tensor, "n batch"] | None = None,
        attn_mask: Bool[Tensor, "batch n"] | None = None,
    ) -> None:
...
class MaskEncoder(nn.Module):
    ...
    def forward(
        self, masks: Float[Tensor, "n c h w"], *args: Any, **kwargs: Any
...
class FusedMaskEncoder(MaskEncoder):
    ...
    def forward(
        self,
        masks: Float[Tensor, "n c h w"],
        pix_feat: Float[Tensor, "n c_pix h_pix w_pix"],
        **kwargs: Any,
    ) -> tuple[Float[Tensor, "..."], Float[Tensor, "..."]]:
        masks = self.mask_downsampler(masks)
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_points(
        self,
        points: Float[Tensor, "n_points batch 2"],
        points_mask: Bool[Tensor, "batch n_points"],
        points_labels: Integer[Tensor, "n_points batch"]
        | Float[Tensor, "n_points batch"],
        img_feats: Float[Tensor, "..."],
    ) -> tuple[Float[Tensor, "..."], Bool[Tensor, "batch n_points"]]:
        points_embed = None
        n_points, bs = points.shape[:2]
        # Match projection weight dtype (Prompt may default empty coords to fp32)
        if self.points_direct_project is not None:
            points = points.to(dtype=self.points_direct_project.weight.dtype)
        ...
        if self.points_pos_enc_project is not None:
            x, y = points.unbind(-1)
            # sine PE stays float32; cast to module dtype for Linear
            enc_x, enc_y = self.pos_enc._encode_xy(
                x.float().flatten(), y.float().flatten()
            )
            enc_x = enc_x.view(n_points, bs, enc_x.shape[-1])
            enc_y = enc_y.view(n_points, bs, enc_y.shape[-1])
            enc = torch.cat([enc_x, enc_y], -1).to(
                self.points_pos_enc_project.weight.dtype
            )
        ...
        if points_embed is not None:
            type_embed = type_embed.to(dtype=points_embed.dtype)
        return type_embed + points_embed, points_mask

    def _encode_boxes(
        self,
        boxes: Float[Tensor, "n_boxes batch 4"],
        boxes_mask: Bool[Tensor, "batch n_boxes"],
        boxes_labels: Integer[Tensor, "n_boxes batch"] | Float[Tensor, "n_boxes batch"],
        img_feats: Float[Tensor, "..."],
    ) -> tuple[Float[Tensor, "..."], Bool[Tensor, "batch n_boxes"]]:
        boxes_embed = None
        n_boxes, bs = boxes.shape[:2]
        if self.boxes_direct_project is not None:
            boxes = boxes.to(dtype=self.boxes_direct_project.weight.dtype)
        ...
        if self.boxes_pool_project is not None:
            ...
            # roi_align requires matching dtypes; run in fp32 then cast back
            feats_f = img_feats.float()
            rois = boxes_xyxy.float().transpose(0, 1).unbind(0)
            sampled = torchvision.ops.roi_align(feats_f, rois, self.roi_size)
            ...
            sampled = sampled.to(self.boxes_pool_project.weight.dtype)
        ...
        if boxes_embed is not None:
            type_embed = type_embed.to(dtype=boxes_embed.dtype)
        return type_embed + boxes_embed, boxes_mask

    def _encode_masks(
        self,
        masks: Float[Tensor, "n_masks batch *rest"],
        attn_mask: Bool[Tensor, "batch n_masks"],
        mask_labels: Integer[Tensor, "n_masks batch"] | Float[Tensor, "n_masks batch"],
        img_feats: Float[Tensor, "..."] | None = None,
    ) -> tuple[Float[Tensor, "..."], Bool[Tensor, "batch tokens"]]:
    ...
    def forward(
        self,
        geo_prompt: Prompt,
        img_feats: list[Float[Tensor, "..."]] | Float[Tensor, "..."],
        img_sizes: list[tuple[int, int]] | list[list[int]],
        img_pos_embeds: list[Float[Tensor, "..."]] | None = None,
    ) -> tuple[Float[Tensor, "..."], Bool[Tensor, "..."]]:
