...
def validate_bundle(bundle_dir: Path) -> dict[str, Any]:
    resolved = validate_manifest_package(
        bundle_dir / "manifests" / f"{AOTINDUCTOR_PLAN_ID}.json",
        expected_plan_id=AOTINDUCTOR_PLAN_ID,
    )
    fixtures = json.loads(
    ...
    session = create_image_session(AOTINDUCTOR_PLAN_ID, bundle_dir=bundle_dir)
    ...
    try:
        for case_index, case in enumerate(fixtures["cases"]):
            ...
            if not indices_match:
                raise RuntimeError(f"{prefix}: admitted query indices do not match")
            if mask_iou < 0.98:
                raise RuntimeError(f"{prefix}: task mask IoU {mask_iou:.6f} < 0.98")
            cases.append(
                {
                    "id": prefix,
        ...
        if counters["image_encodes"] != len(fixtures["images"]):
            raise RuntimeError(f"image cache reuse gate failed: {counters}")
        if counters["text_encodes"] != len(fixtures["cases"]):
            raise RuntimeError(f"text cache reuse gate failed: {counters}")
    finally:
        session.close()
...
def main() -> int:
    ...
    report_path = bundle_dir / "reports/m6_aotinductor_validation.json"
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    manifest_path = bundle_dir / "manifests" / f"{AOTINDUCTOR_PLAN_ID}.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    report_ref = _file_id("reports/m6_aotinductor_validation.json")
